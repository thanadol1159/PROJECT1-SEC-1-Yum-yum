...
<template>
  <div class="w-screen h-screen">
    <!-- BG Fix -->
    <div
      class="bg-bottom bg-cover bg-no-repeat w-full h-full bg-[url('img/bg-fix-fix.png')]"
    >
      <!-- BG Blur -->
      <div
        class="absolute w-screen h-screen bg-bottom bg-cover bg-no-repeat bg-[url('img/bg-blur.jpg')]"
        @click="bg_first_func"
        v-show="bg_first"
      >
        <h1
          class="w-full flex justify-center mt-9 font-sans font-extrabold text-9xl tracking-wider text-white"
        >
          BLACKJACK
        </h1>
        <h4
          class="w-full font-sans font-semibold text-3xl flex justify-center text-end mt-64 text-gray-300"
        >
          click to start
        </h4>
      </div>
        ...
        >
          ...
          <div>
            <ul>
              ...
              <li class="pb-16 mt-3">
                <a
                  class="absolute left-5 text-5xl hover:text-slate-400"
              ...
              <li>
                <img
                  class="w-24 m-auto"
                  src="../src/assets/img/back_card.png"
                />
              </li>
              <li>
                <a
                  class="pt-8 text-center text-6xl text-slate-200 block hover:text-zinc-400"
                  @click="MenuOpen()"
                  >RESUME</a
                >
              </li>
          ...
          <div class="flex pr-2 pt-36 justify-center flex-row">
            ...
            <!-- stop start music -->
            <label
              :class="onOff === 1 ? 'text-orange-400' : ' text-white'"
              class="text-2xl pl-5 -top-1.5 cursor-pointer"
              for="on"
              >on</label
            >
            <input
            ...
            <label
              :class="onOff === 0 ? 'text-orange-400' : ' text-white'"
              class="text-2xl pl-5 -top-1.5 cursor-pointer"
              for="off"
              >off</label
            >
      ...
      <!-- Score -->
      <div
        class="flex justify-center items-center text-stone-900"
        v-show="!bg_first"
      >
        <div class="relative flex justify-center">
          <div
            class="totalScore z-10 relative flex items-center justify-center"
          >
            <span class="pt-10"> Round: {{ numRound }} </span>
          </div>
          <div class="absolute top-0 m-auto z-30 round">
            <span
              class="compare flex justify-center items-center w-60 text-2xl font-bold"
            >
          ...
          <div
            class="player absolute m-auto z-20 flex items-center top-0 bx6 justify-around"
          >
      ...
      <div class="w-full pt-6">
        <div class="flex justify-center space-x-5" ref="imgCardDealer">
          <img
            v-if="showHit"
            src="img/All-card-final/backcard/back_card.svg"
            class="w-32"
          />
